...
public:
	Central_Field() : Function("eq1.png", "Central"), a{ 1 }, b{ 1 }, c{ 1 } {}
...
public:
	Sinusoidal_Field() : a{ 1 }, b{ 1 }, c{ 1 }, Function("eq2.png", "Sinusoidal") {}
	void Transform(double& x, double& y, double& z) {
		double r = sqrt(x * x + y * y + z * z);
		double buff_x = x;
		x *= c * (sin(a * x) + sin(a * y));
...
public:
	Vortex_Field() : a{ 1 }, b{ 1 }, c{ 1 }, Function("eq3.png", "Vortex") {}
	void Transform(double& x, double& y, double& z) {
		double r = sqrt(x * x + y * y + z * z);
		double x_buff = x; 
		double y_buff = y;
